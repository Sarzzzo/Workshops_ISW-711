...
// esta funcion de arriba es la funcion de autorizacion que lleva en el bearer el token
// rescatado del localStorage. Si no hay token, redirige al login
...
async function cargarProfesores() {
    try {
        ...
        if (!headers) return;
        const response = await fetch(`${API_URL}/profesores`, { headers });
...
async function editarProfesor(id) {
    try {
        const headers = getAuthHeader();
        if (!headers) return;
        const response = await fetch(`${API_URL}/profesores/${id}`, { headers });
...
async function editarCurso(id) {
    try {
        const headers = getAuthHeader();
        if (!headers) return;
        const response = await fetch(`${API_URL}/cursos/${id}`, { headers });
